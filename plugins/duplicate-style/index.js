...
OptimizeCssAssetsPlugin.prototype.print = function () {
  if (this.options.canPrint || (typeof this.options.canPrint === 'undefined' && (process.env.NODE_ENV === 'production' || process.env.__TOUCHUI_BUILD__))) {
...
OptimizeCssAssetsPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', function (compilation, compileCallback) {
    self.print('\n\n======== touchui-loader: duplicate-style start~  ========')
    ...
    Promise.all(promises).then(function () {
      compileCallback()
      self.print('======== touchui-loader: duplicate-style done!   ========\n')
    }, compileCallback)
  })
}
